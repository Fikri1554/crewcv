...
class Contract extends CI_Controller {
	...
	function getDataCrewStatus($search = "")
	{
		...
		if($search != "")
		{
			...
			if($status == "onboard"  OR $status == "all")
			{
				...
				$sql = "SELECT A.idcontract,A.idperson,TRIM(CONCAT(D.fname,' ',D.mname,' ' ,D.lname)) AS fullName,d.usecertdoc,D.dob,A.kdcmprec,A.signondt,A.signoffdt,A.estsignoffdt,A.signonrank,B.nmrank,A.signonvsl,C.kdvsl,C.nmvsl,A.lastvsl,A.signondesc,F.NmNegara
					FROM tblcontract A
				...
				{
					$dataTemp[$val->kdvsl][$keyNo]['idcontract'] = $val->idcontract;
					$dataTemp[$val->kdvsl][$keyNo]['idperson'] = $val->idperson;
					$dataTemp[$val->kdvsl][$keyNo]['fullName'] = $val->fullName;
					$dataTemp[$val->kdvsl][$keyNo]['usecertdoc'] = $val->usecertdoc;
					$dataTemp[$val->kdvsl][$keyNo]['dob'] = $val->dob;
					$dataTemp[$val->kdvsl][$keyNo]['kdcmprec'] = $val->kdcmprec;
					$dataTemp[$val->kdvsl][$keyNo]['signondt'] = $val->signondt;
					$dataTemp[$val->kdvsl][$keyNo]['signoffdt'] = $val->signoffdt;
					$dataTemp[$val->kdvsl][$keyNo]['estsignoffdt'] = $val->estsignoffdt;
					$dataTemp[$val->kdvsl][$keyNo]['signonrank'] = $val->signonrank;
					$dataTemp[$val->kdvsl][$keyNo]['nmrank'] = $val->nmrank;
					$dataTemp[$val->kdvsl][$keyNo]['signonvsl'] = $val->signonvsl;
					$dataTemp[$val->kdvsl][$keyNo]['nmvsl'] = $val->nmvsl;
					$dataTemp[$val->kdvsl][$keyNo]['lastvsl'] = $val->lastvsl;
					$dataTemp[$val->kdvsl][$keyNo]['signondesc'] = $val->signondesc;
					$dataTemp[$val->kdvsl][$keyNo]['NmNegara'] = $val->NmNegara;
					$dataTemp[$val->kdvsl][$keyNo]['certName'] = $this->getcertificateByUser($val->idperson);
				...
				foreach ($dataTemp as $key => $value)
				{
					foreach ($value as $key => $val)
					{
						...
						$tempTr .= "<tr>";
							$tempTr .= "<td style=\"background-color:#5bc0de;font-size:11px;text-align:center;width:5%;\">".$no."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:10%;\">".$val['nmrank']."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:15%;\">".$val['fullName']."&nbsp".$bintang.$warning."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:10%;\">".$val['NmNegara']."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:center;width:10%;\">".$dataContext->convertReturnName($val['dob'])."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:10%;\">".$val['lastvsl']."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:center;width:10%;\">".$dataContext->convertReturnName($val['signondt'])."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:center;width:10%;\">".$dataContext->convertReturnName($val['signoffdt'])."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:center;width:10%;\">".$dataContext->convertReturnName($val['estsignoffdt'])."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:10%;\">".$val['certName']['certPanama']."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:10%;\">".$val['certName']['certOther']."</td>";
							$tempTr .= "<td style=\"background-color:#D7EAEC;font-size:11px;text-align:left;width:10%;\">".$btnAct."</td>";
						$tempTr .= "</tr>";
				...
				$trNya .= "<tr>";
					$trNya .= "<td style=\"background-color:#5bc0de;\" colspan=\"24\">";
			...
			if($status == "onleave"  OR $status == "all")
			{
				...
				$trNya .= "<tr><td style=\"background-color:#5CB85C;color:#FFF;\" colspan=\"24\"><b><i>:: ON LEAVE ::</i> ( ".number_format($no-1,0)." Data ) </b></td></tr>";
				$trNya .= $tempTr;
			}
			...
			if($status == "nonaktif" OR $status == "all")
			{
				...
				$trNya .= "<tr><td style=\"background-color:#F0AD4E;\" colspan=\"24\"><b><i>:: NON AKTIF ::</i> ( ".number_format($no-1,0)." Data ) </b></td></tr>";
				$trNya .= $tempTr;
			}
			if($status == "notforemp"  OR $status == "all")// blacklist
			{
				...
				$trNya .= "<tr><td style=\"background-color:#D9534F;color:#FFF;\" colspan=\"24\"><b><i>:: NOT FOR EMP ::</i> ( ".number_format($no-1,0)." Data ) </b></td></tr>";
				$trNya .= $tempTr;
			}			
			
		}else{
			$trNya = "<tr><td style=\"text-align:center;font-weight:bold;\" colspan=\"24\">- Select Status -</td></tr>";
		}
	...
	function getData($idPerson = "")
	{
		...
		foreach ($rsl as $key => $val)
		{
			...
			if($val->file_contract != "")
			{
				$imgContract = "<a href=\"".base_url('uploadFileContract/'.$val->file_contract)."\" target=\"_blank\" class=\"btn btn-info btn-xs btn-block\" style=\"margin-top:5px;\" title=\"File Contract\"><i class=\"fa fa-file\"></i></a>";
			}
			...
			$trNya .= "<tr>";
				$trNya .= "<td style=\"font-size:10px;text-align:center;\">".$no.$imgContract."</td>";
	...
	function saveData()
	{
		...
		try {
			...
			if($data['cekFileContract'] != "")
			{
				$fileUploadNya = "";
				$fileName = $_FILES["fileContract"]["name"];
				$newFileName = "fileContract_".$idPerson."_".$idEdit;
				$fileUploadNya = $dataContext->uploadFile($_FILES["fileContract"]['tmp_name'],$dir,$fileName,$newFileName);
				$dataIns['file_contract'] = $fileUploadNya;
	...
	function getDataEdit()
	{
		...
		if(count($rsl) > 0)
		{
			...
			if($rsl[0]->file_contract != "")
			{
				$imgContract = "<a href=\"".base_url('uploadFileContract/'.$rsl[0]->file_contract)."\" target=\"_blank\" class=\"btn btn-success btn-xs btn-block\" style=\"margin-top:5px;\">View</a>";
				$imgContract .= "<button class=\"btn btn-danger btn-xs btn-block\" onclick=\"delFileContract('".$rsl[0]->idcontract."','".$rsl[0]->file_contract."','contract');\">Delete</button>";
			}
			...
			$dataOut['imgContract'] = $imgContract;
		}
		...
		if(count($rsl) > 0)
		{
			foreach ($rsl as $key => $val)
			{
				if ($val->kdnegara == '021') { 
					...
					$tempData['panama'][$keyNo]['certName'] = $val->certname;
					$tempData['panama'][$keyNo]['issDate'] = $val->issdate;
					$tempData['panama'][$keyNo]['expDate'] = $val->expdate;
					$tempData['panama'][$keyNo]['certFile'] = $val->certificate_file;
				} else {
					$tempData['other'][$keyNo]['certName'] = $val->certname;
					$tempData['other'][$keyNo]['issDate'] = $val->issdate;
					$tempData['other'][$keyNo]['expDate'] = $val->expdate;
					$tempData['other'][$keyNo]['certFile'] = $val->certificate_file;
				}
			...
			foreach ($tempData['panama'] as $key => $val)
			{
				...
				if($certPanama == "")
				{
					$certPanama = "- ".$val['certName'];

					if($val['certFile'] != "")
					{
						$certPanama = "- <a href=\"".base_url('uploadCertificate/'.$val['certFile'])."\" target=\"_blank\" style=\"margin-top:5px;\">".$val['certName']."</a>";
					}		

					$certPanama .= "<br>&nbsp<label>(Iss. Date : ".$issDate.", Exp. Date : ".$expDate.")</label>";
				}else{							
					if($val['certFile'] != "")
					{
						$certPanama .= "<br>- <a href=\"".base_url('uploadCertificate/'.$val['certFile'])."\" target=\"_blank\" style=\"margin-top:5px;\">".$val['certName']."</a>";
					}else{
						$certPanama .= "<br>- ".$val['certName'];
					}
			...
			foreach ($tempData['other'] as $key => $val)		
			{
				$issDate = $dataContext->convertReturnName($val['issDate']);
				$expDate = $dataContext->convertReturnName($val['expDate']);
				...
				if($certOther == "")
				{
					$certOther = "- ".$val['certName'];

					if($val['certFile'] != "")
					{
						$certOther = "- <a href=\"".base_url('uploadCertificate/'.$val['certFile'])."\" target=\"_blank\" style=\"margin-top:5px;\">".$val['certName']."</a>";
					}		

					$certOther .= "<br>&nbsp<label>(Iss. Date : ".$issDate.", Exp. Date : ".$expDate." )</label>";
				}else{							
					if($val['certFile'] != "")
					{
						$certOther .= "<br>- <a href=\"".base_url('uploadCertificate/'.$val['certFile'])."\" target=\"_blank\" style=\"margin-top:5px;\">".$val['certName']."</a>";
					}else{
						$certOther .= "<br>- ".$val['certName'];
					}
	...
	function deleteFileContract()
	{
		...
		$dataDel['file_contract'] = "";

		$whereNya = "idcontract = '".$id."'";
		$this->MCrewscv->updateData($whereNya,$dataDel,"tblcontract");

		$dataContext->delFile($fileName,$dir);
